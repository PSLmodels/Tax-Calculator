...
def main():
    ...
    calc3p.records = all_earnings_shift(calc3p.records, taxpayer_only=True)
    calc3p.calc_all()
    records3b = Records()
    policy3b = Policy()
    ...
    calc3b.records = all_earnings_shift(calc3b.records, taxpayer_only=False)
    calc3b.calc_all()
...
def shift_earnings(recs, does_p, does_s):
    """
    ...
    """
    # shift for taxpayers
    recs.e02000 = np.where(does_p, recs.e02000 + recs.e00200p, recs.e02000)
    recs.e26270 = np.where(does_p, recs.e26270 + recs.e00200p, recs.e26270)
    recs.k1bx14p = np.where(does_p, recs.k1bx14p + recs.e00200p, recs.k1bx14p)
    recs.e00200 = np.where(does_p, recs.e00200 - recs.e00200p, recs.e00200)
    recs.e00200p = np.where(does_p, 0., recs.e00200p)
    # shift for spouses
    recs.e02000 = np.where(does_s, recs.e02000 + recs.e00200s, recs.e02000)
    recs.e26270 = np.where(does_s, recs.e26270 + recs.e00200s, recs.e26270)
    recs.k1bx14s = np.where(does_s, recs.k1bx14s + recs.e00200s, recs.k1bx14s)
    recs.e00200 = np.where(does_s, recs.e00200 - recs.e00200s, recs.e00200)
    recs.e00200s = np.where(does_s, 0., recs.e00200s)
    return recs


def all_earnings_shift(recs, taxpayer_only):
    """
    Return Records object with wages and salaries in recs shifted to
    ...
    recs = shift_earnings(recs, does, noes)
    if taxpayer_only is False:
        # also do earnnigs shift for all spouses
        recs = shift_earnings(recs, noes, does)
    if dump:
        data = [getattr(recs, col) for col in cols]
        pdf = pd.DataFrame(data=np.column_stack(data), columns=cols)

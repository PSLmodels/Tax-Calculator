...
def test_ext_plus_ctc_reform(tests_path):
    ...
    with open(filename, 'r', encoding='utf-8') as rfile:
        rtext = rfile.read()
    bas.implement_reform(Policy.read_json_reform(rtext))
    ...
    for jsonname in ['ext.json', 'ctc.jsonx']:
        ...
        with open(filename, 'r', encoding='utf-8') as rfile:
            rtext = rfile.read()
        ref.implement_reform(Policy.read_json_reform(rtext))
        assert not ref.parameter_errors
    # create Calculator objects using public CPS data
    recs = Records.cps_constructor()
    calc_bas = Calculator(policy=bas, records=recs)
    calc_ref = Calculator(policy=ref, records=recs)
    # check change in taxes generated by the compound reform in several years
    expected_wdiffs = {
        2025: 0.0,
        2027: -31.1,
        2029: 0.0,
    ...
    for year, expected_wdiff in expected_wdiffs.items():
        calc_bas.advance_to_year(year)
        calc_bas.calc_all()
        calc_ref.advance_to_year(year)
        calc_ref.calc_all()
        wdiff = (
            (calc_ref.array('iitax') - calc_bas.array('iitax')) *
            calc_bas.array('s006')
        ).sum()
        assert np.allclose(
            [round(wdiff * 1e-9, 1)],

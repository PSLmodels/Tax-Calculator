...
@pytest.fixture(scope='module', name='reformfilex')
def fixture_reformfilex():
    """
    Temporary reform file with .json extension.
    """
    rfile = tempfile.NamedTemporaryFile(suffix='.json', mode='a', delete=False)
...
@pytest.mark.parametrize("year, ref, asm, gdr", [
    ...
    (2020, 'reformfilex', 'assumpfile0', 'has_gdiff_response'),
])
def test_init_errors(reformfile0, reformfilex, assumpfile0,
                     year, ref, asm, gdr):
    """
    ...
    elif ref == 'reformfilex':
        reform = reformfilex.name
    else:
        reform = ref
    if asm == 'assumpfile0':

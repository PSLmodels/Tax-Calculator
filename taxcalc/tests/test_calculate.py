...
def test_ID_HC_vs_BS(cps_subsample, puf_subsample):
    ...
    # TODO: clean up below
    hc = hc_calc.array('iitax')
    bs = bs_calc.array('iitax')
    id = hc_calc.array('RECID')
    eps = 0.01
    for idx in range(0, hc.size):
        diff = hc[idx] - bs[idx]
        if diff < -eps or diff > eps:
            print idx, id[idx], hc[idx], bs[idx], diff
    # TODO: clean up above

...
def translate(ivar):
    ...
    invar['PT_SSTB_income'] = 0#np.where(invar['pprofinc'] + invar['sprofinc'] > 0, 1, 0)
    invar['PT_SSTB_income'] = 0#np.where(invar['e26270'] > 0, 1, invar['PT_SSTB_income'])

    # Drop TAXSIM variables not used in Tax-Calculator
    invar.drop(columns=['state', 'mstat', 'depx', 'dep18', 'transfers', 'rentpaid', 'pprofinc', 'sprofinc'], inplace=True)

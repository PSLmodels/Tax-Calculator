...
if os.path.exists(os.path.join(CURR_PATH, taxsim_in)) is False:
    sys.exit("LYY_FILENAME is not a valid path")
...
def prep_tc_input():
    command = "python prepare_taxcalc_input.py " + taxsim_in + " " + taxsim_in_csv
    os.system(command)

# calculate Tax-Calculator output
def calc_tc_output():
    year  = '20' + YY
    command = "tc " + taxsim_in_csv + " " + year + " --reform taxsim_emulation.json --dump"
    os.system(command)
...
def convert_to_taxsim():
    ...
    command = "python process_taxcalc_output.py " + taxsim_out_csv + " " + file_out
    os.system(command)

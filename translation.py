...
# global dim 
# dim = len(puf_dict)
# Make this programmatically available
# dim = 139651
...
def Puf(master_puf):
	# dim = len(master_puf)
	with open('zeros.json') as defaults_file:
		default_var_names = json.load(defaults_file)
	default_var_dict = dict((var, DIMARRAY) 
...
def AMTI(puf):
	...
	_cmbtp = np.where(np.logical_and(puf == True, np.logical_and(np.logical_or(_standard == 0, np.logical_and(puf_dict['exact'] == 1, puf_dict['e04470'] > 0)), puf_dict['f6251'] == 1)), -1 * np.minimum(_edical, 0.025 * np.maximum(0, puf_dict['e00100'])) + puf_dict['e62100'] + c60260 + puf_dict['e04470'] + puf_dict['e21040'] - _sit - puf_dict['e00100'] - puf_dict['e18500'] - puf_dict['e20800'], _cmbtp)
